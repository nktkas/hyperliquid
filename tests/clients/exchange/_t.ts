    ...
    Deno.test(name, async (t) => {
        ...
        if (topup) {
            if (topup.perp) {
                ...
                await mainExchClient.usdSend({
                    destination: multiSignClient.multiSignAddress,
                    amount: topup.perp,
                });
            }
            if (topup.spot) {
        ...
        try {
            ...
            await Promise.all([
                ...
                infoClient.clearinghouseState({ user: multiSignClient.multiSignAddress })
                    .then(async (state) => {
...
export async function getAssetData(
    assetName: string,
): Promise<{ id: number; universe: PerpsUniverse; ctx: PerpsAssetCtx }> {
