...
export type MethodReturnType = Awaited<ReturnType<MultiSignClient["usdSend"]>>;
...
Deno.test("usdSend", { ignore: !PRIVATE_KEY }, async () => {
    ...
    const data = await multiSignClient.usdSend({ destination: DESTINATION_ADDRESS, amount: "1" });
