...
export type MethodReturnType = Awaited<ReturnType<MultiSignClient["perpDexClassTransfer"]>>;
const MethodReturnType = schemaGenerator(import.meta.url, "MethodReturnType");

// —————————— Test ——————————

Deno.test("perpDexClassTransfer", { ignore: !PRIVATE_KEY }, async () => {
    ...
    const data = await Promise.all([
        multiSignClient.perpDexClassTransfer({ dex: "test", token: "USDC", amount: "1", toPerp: true }),
        multiSignClient.perpDexClassTransfer({ dex: "test", token: "USDC", amount: "1", toPerp: false }),
    ]);
