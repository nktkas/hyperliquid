...
export type MethodReturnType = Awaited<ReturnType<MultiSignClient["reserveRequestWeight"]>>;
const MethodReturnType = schemaGenerator(import.meta.url, "MethodReturnType");

// —————————— Test ——————————

Deno.test("reserveRequestWeight", { ignore: !PRIVATE_KEY }, async () => {
    ...
    const data = await Promise.all([
        ...
        multiSignClient.reserveRequestWeight({ weight: 1 }),
        // Check argument 'expiresAfter'
        multiSignClient.reserveRequestWeight({ weight: 1, expiresAfter: Date.now() + 1000 * 60 * 60 }),
    ]);
