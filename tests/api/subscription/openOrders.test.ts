import { OpenOrdersEvent } from "@nktkas/hyperliquid/api/subscription";
import { collectEventsOverTime, runTest } from "./_t.ts";
import { schemaCoverage } from "../_schemaCoverage.ts";

runTest({
  name: "openOrders",
  mode: "api",
  fn: async (_t, client) => {
    const data = await collectEventsOverTime<OpenOrdersEvent>(async (cb) => {
      await client.openOrders({ user: "0x563C175E6f11582f65D6d9E360A618699DEe14a9" }, cb);
    }, 10_000);
    schemaCoverage(OpenOrdersEvent, data, {
      ignorePicklistValues: {
        "#/properties/orders/items/properties/orderType": ["Market", "Take Profit Market", "Take Profit Limit"],
        "#/properties/orders/items/properties/tif/wrapped": ["Ioc", "FrontendMarket", "LiquidationMarket"],
      },
    });
  },
});
